...
public class CreateBlockCommandHandler : IRequestHandler<CreateBlockCommand, Result<Guid>>
{
    ...
    public async Task<Result<Guid>> Handle(CreateBlockCommand request, CancellationToken cancellationToken)
    {
        ...
        // Start explicit transaction for block creation + file move
        ...
        try
        {
            ...
            {
                // Build destination path: tenant-{TenantId}/blocks/{BlockId}/{FileName}
                var destinationPath = $"tenant-{tenantId.Value}/blocks/{block.Id}/{tempFile.FileName}";
                ...
                var document = Document.Create(
                    ownerType: DocumentOwnerType.Block,
                    ownerId: block.Id,
        ...
        catch (Exception ex)
        {
            ...
            return Result<Guid>.Failure($"Failed to create block with images: {ex.Message}");
        }
    }
}
