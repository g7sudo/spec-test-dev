...
public class CreateFloorCommandHandler : IRequestHandler<CreateFloorCommand, Result<Guid>>
{
    ...
    public async Task<Result<Guid>> Handle(CreateFloorCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                // Build destination path: tenant-{TenantId}/floors/{FloorId}/{FileName}
                var destinationPath = $"tenant-{tenantId.Value}/floors/{floor.Id}/{tempFile.FileName}";
        ...
        catch (Exception ex)
        {
            ...
            return Result<Guid>.Failure($"Failed to create floor with images: {ex.Message}");
        }
    }
}
