...
/// <summary>
/// Handler for listing floors with optional block filtering and pagination.
/// </summary>
public class ListFloorsQueryHandler : IRequestHandler<ListFloorsQuery, Result<PagedResult<FloorDto>>>
{
    ...
    public async Task<Result<PagedResult<FloorDto>>> Handle(ListFloorsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (request.BlockId.HasValue)
        {
            ...
            var blockExists = await _dbContext.Blocks
                .AsNoTracking()
                .AnyAsync(b => b.Id == request.BlockId.Value && b.IsActive, cancellationToken);

            if (!blockExists)
            {
                return Result<PagedResult<FloorDto>>.Failure($"Block with ID '{request.BlockId.Value}' not found.");
            }
        ...
        // Get block names for all floors
        var blockIds = items.Select(x => x.BlockId).Distinct().ToList();
        var blockNames = await _dbContext.Blocks
            .AsNoTracking()
        ...
        foreach (var floor in items)
        {
            ...
            floorDtos.Add(new FloorDto
            {
                ...
                BlockName = blockNames.GetValueOrDefault(floor.BlockId, string.Empty),
                Name = floor.Name,
